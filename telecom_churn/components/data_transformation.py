...
from telecom_churn.constants import TARGET_COLUMN, SCHEMA_FILE_PATH
from telecom_churn.entity.config_entity import DataTransformationConfig
from telecom_churn.entity.artifact_entity import DataTransformationArtifact, DataIngestionArtifact, DataValidationArtifact
from telecom_churn.exception import TelecomChurnException
from telecom_churn.logger import logging
from telecom_churn.utils.main_utils import save_object, save_numpy_array_data, read_yaml_file, drop_columns
from telecom_churn.entity.estimator import TargetValueMapping
...
class DataTransformation:
    ...
    def get_data_transformer_object(self) -> Pipeline:
        ...
        try:
            ...
            or_columns = self._schema_config['ord_columns']
    ...
    def initiate_data_transformation(self, ) -> DataTransformationArtifact:
        ...
        try:
            if self.data_validation_artifact.validation_status:
                ...
                logging.info("Adjusts the TotalCharges variable and converts its data type")

                idx_remove = train_df[train_df['TotalCharges'] == ' '].index.to_list()
                train_df.drop(index=idx_remove, inplace=True)

                train_df['TotalCharges'] = train_df['TotalCharges'].astype(float)

                idx_remove = test_df[test_df['TotalCharges'] == ' '].index.to_list()
                test_df.drop(index=idx_remove, inplace=True)

                test_df['TotalCharges'] = test_df['TotalCharges'].astype(float)

                logging.info("Separates the predictor variables from the target variable")
